...
from utils import MaskedNLLLoss, MELDRobertaCometDataset
from model import Kamuh
from sklearn.metrics import f1_score, accuracy_score

def create_class_weight(a=1):
    emotions_numerical = [0, 1, 2, 3, 4, 5, 6]
    emotions_dict = {0: 6436, 1: 1636, 2: 358, 3: 1002, 4: 2308, 5: 361, 6: 1607}
    total = np.sum(list(emotions_dict.values()))
    weights = []
    for key in emotions_numerical:
        score = math.log(a*total/emotions_dict[key])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', type=float, default=0.001, help='hyperparameter for multimodal gate')
    ...
    model = Kamuh(D_m, D_s, D_g, D_p, D_r, D_i, D_e, D_h, D_a,
                                n_classes=n_classes,
    ...
    if args.classify == 'emotion':
        lf = open('logs/kamuh_meld_emotion_logs.txt', 'a')
    elif args.classify == 'sentiment':
        lf = open('logs/kamuh_meld_sentiment_logs.txt', 'a')
    ...
    if args.classify == 'emotion':
        rf = open('results/kamuh_meld_emotion_results.txt', 'a')
    elif args.classify == 'sentiment':
        rf = open('results/kamuh_meld_sentiment_results.txt', 'a')
